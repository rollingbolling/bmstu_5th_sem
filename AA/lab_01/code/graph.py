...
def create_graph(algo_1, algo_2, algo_3, algo_4, seconds, length):
    ...
    ax.set_ylabel("Время выполнения (миллисекунды)")
...
with open('measure_long.csv', 'r') as datafile:
    ...
    for ROWS in plotting:
        Sec2.append(float(ROWS[0]))
        NoRec.append(float(ROWS[1]) * 1000)
        Rec.append(float(ROWS[2]) * 1000)
        RecCash.append(float(ROWS[3]) * 1000)
        DamNoRec.append(float(ROWS[4]) * 1000)
